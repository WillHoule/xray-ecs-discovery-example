...
describe('Subsegment', function() {
  ...
  describe('#close', function() {
    var sandbox, segment, stubSegmentDecrement, stubSubsegmentStream, subsegment;

    beforeEach(function() {
      sandbox = sinon.sandbox.create();

      segment = { parent_id: '12345abc3456def', counter: 1, decrementCounter: function() {}, isClosed: function() {} };
      stubSegmentDecrement = sandbox.stub(segment, 'decrementCounter');

      subsegment = new Subsegment('child');
      subsegment.parent = segment;
      subsegment.segment = segment;
      stubSubsegmentStream = sandbox.stub(subsegment, 'streamSubsegments');
    });
  ...
  describe('#streamSubsegments', function() {
    var child, grandchild, parent, sandbox, stubFlush, stubGrandchildStream;

    beforeEach(function() {
      sandbox = sinon.sandbox.create();
      SegmentUtils.setStreamingThreshold(1);

      parent = new Subsegment('parent');
      child = parent.addNewSubsegment('child');
      grandchild = child.addNewSubsegment('grandchild');

      stubFlush = sandbox.stub(child, 'flush');
      sandbox.stub(grandchild, 'flush');
      stubGrandchildStream = sandbox.stub(grandchild, 'streamSubsegments');
    });
    ...
    describe('if the child subsegment is closed and has no open subsegments', function() {
      it('should flush the subsegment', function() {
        child.close();
        grandchild.close();
        child.streamSubsegments();

        stubFlush.should.have.been.calledOnce;
      });

      it('should remove the child from its parents subsegment array', function() {
        child.close();
        grandchild.close();
        child.streamSubsegments();

        assert.deepEqual(parent.subsegments, []);
      });
    });

    describe('if the child subsegment not closed', function() {
      it('should call streamSubsegment on each subsegment', function() {
        child.streamSubsegments();

        stubGrandchildStream.should.have.been.calledOnce;
      });
    });

    describe('if the child subsegment has open subsegments', function() {
      it('should call streamSubsegment on each subsegment', function() {
        child.counter = 10;
        child.streamSubsegments();
