...
Subsegment.prototype.flush = function flush() {
  ...
  if (this.segment.trace_id) {
    if (this.segment.notTraced !== true) {
      ...
      logger.getLogger().info('Segment marked as not sampled. Ignoring.');
    }
  } else {
    logger.getLogger().info('Segment missing a trace ID. Ignoring.');
  }
};
...
Subsegment.prototype.streamSubsegments = function streamSubsegments() {
  if (this.isClosed() && this.counter <= 0) {
    ...
    var index = _.indexOf(this.parent.subsegments, this);
    this.parent.subsegments.splice(index, 1);
  } else {
    _.each(this.subsegments, function(child) {
