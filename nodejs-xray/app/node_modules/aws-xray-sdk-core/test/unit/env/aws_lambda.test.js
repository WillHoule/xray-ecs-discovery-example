...
describe('AWSLambda', function() {
  ...
  describe('FacadeSegment', function() {
    ...
    describe('#addSubsegment', function() {
      var addSubsegmentStub, facade, resolveStub, sandbox;

      beforeEach(function() {
        sandbox = sinon.sandbox.create();
        process.env._X_AMZN_TRACE_ID = 'traceId;traceId;traceId';
        Lambda.init();

        facade = setSegmentStub.args[0][0];
        addSubsegmentStub = sandbox.stub(facade, '__addSubsegment');
        resolveStub = sandbox.stub(facade, 'resolveLambdaTraceData');
      });
      ...
      it('should call resolveLambdaTraceData', function() {
        facade.addSubsegment({});
        resolveStub.should.have.been.calledOnce;
      });

      it('should call __addSubsegment', function() {
        facade.addSubsegment({});
        addSubsegmentStub.should.have.been.calledOnce;
      });
    });
  });
});
